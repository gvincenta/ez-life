...
export default class SuggestFurther extends Component {
    ...
    handleOnChange = (res) =>{
        ...
        var cat = res[1];
        var isIncome = res[0];
        this.props.axios.post("/report/oneCategory", {name: res[1]})
      .then(function(response) {
        var d = response.data;
    ...
    render(){
        ...
        return(
            ...
            <Cascader options={this.state.option} placeholder="Please select" onChange= {this.handleOnChange}/>
            {(oneCat !== "")
            ? <div>  <h2> Overview of {catName} for the past 1 year:</h2><Line data = {oneCat}/> </div>
            :<hr/>
            }
            </div>
        ) 
