...
class TransList extends React.Component {
            ...
            dataSource: [
                {
                    ...
                    date: {
                        ...
                        display: moment("2019-05-01")
                    }, // "dateValue" is to display on the table as a string, "display" is to display on the selector as defaultValue
                    amount: 120,
                    category: {
                ...
                render: (date, record) => {
                    return record.type !== "view" ? (
                        <DatePicker
                            format="YYYY-MM-DD"
                            defaultValue={
                                record.date.display || moment(new Date())
                            }
                    ...
                    <span>
                        {record.type === "new" && (
                            <span>
                                <a
                                    href="javascript:;"
                                    onClick={e => this.addSubmit(record)}
                                >
                                    Add
                                </a>
                                ...
                                <a
                                    href="javascript:;"
                                    onClick={e => this.cancelAdd(record)}
                                >
                                    Cancel
                                </a>
                            </span>
                        ...
                        {record.type === "edit" && (
                            <span>
                                <a
                                    href="javascript:;"
                                    onClick={e => this.editSubmit(record)}
                                >
                                    Save
                                </a>
                                <Divider type="vertical" />
                                <a
                                    href="javascript:;"
                                    onClick={e => this.cancelEdit(record)}
                                >
                                    Cancel
                                </a>
                            </span>
    ...
    categoryChange(e, record) {
        if (e.length === 0) {
            ...
            const display = e;
            this.updateEditCacheData(record, {
                category: { cateValue, display }
            });
        }
    }
    ...
    updateDataSource(newData, isAddDisabled) {
        let isRowOpen =
            typeof isAddDisabled == "boolean"
                ? isAddDisabled
                : newData.some(
                      item => item.type === "new" || item.type === "edit"
                  );
    ...
    addRow = () => {
        ...
        let newRecord = {
            ...
            date: {
                dateValue: currDate.format("YYYY-MM-DD"),
                display: currDate
            },
            amount: 1,
            category: {},
