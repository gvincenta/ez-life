...
class EditableTable extends React.Component {
  ...
  handleDelete = (key) => {
    ...
    this.setState({ transactions: transactions.filter(item => item.key !== key) });
  }
  ...
  categoryOption = () =>{
    ...
    this.props.axios
          ...
          .then(function(response) {
            var answer = [{value : "income", label: "income", children : []},
            ...
            for (var i = 0; i < d.length ; i++){
              if (d[i].isIncome === "needs" ){  
                answer[1].children.push( {value : d[i].name,label : d[i].name, })
              }
              else if (d[i].isIncome === "income" ){
                answer[0].children.push( {value : d[i].name,label : d[i].name, })
              }
              else if (d[i].isIncome === "wants" ){
                answer[2].children.push( {value : d[i].name,label : d[i].name, })
              }
  ...
  render() {
    ...
    const item = (editingKey === '-1' ? {} : transactions.find(item => item.key === editingKey));
    ...
    return (
      <div>
        ...
        <Button  data-step = '2' data-intro = "Add a new transaction" type="primary" onClick={this.showModal}>
          New Transaction
        </Button>
        <TransFormTable 
